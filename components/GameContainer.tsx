...
export default function GameContainer({ puzzlePackage }: GameContainerProps) {
  const {
    gameState,
    currentPuzzle,
    checkGuess,
  ...
  // Determine which image to show based on stage (0-6)
  // Ensure we don't go out of bounds if something goes wrong
  const currentImageIndex = Math.min(gameState.currentStage, 6);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-2 flex justify-between items-end">
          ...
          <span className="text-xs opacity-50">
            {7 - gameState.currentStage} attempts remaining for this puzzle
          </span>
        </div>
        ...
        <div className="space-y-8">
          <VerseImage
            ...
            revealed={gameState.isCorrect === true}
          />
          ...
          {gameState.isCorrect !== true ? (
            <div className="flex justify-center">
              <GuessInput
                onGuess={handleGuess}
                onSkip={skipVerse}
                showHint={gameState.currentStage > 2} // Only show text hint after 3 fails
